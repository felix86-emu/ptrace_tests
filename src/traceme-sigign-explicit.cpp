...
int tracee_main(pid_t parent) {
    // Like the traceme-sigign test, but explicitly set a signal to SIGIGN
    struct sigaction sa;
    sa.sa_sigaction = (decltype(sa.sa_sigaction))SIG_IGN;
    sa.sa_flags = 0;
    sigemptyset(&sa.sa_mask);
    ASSERT(sigaction(SIGPWR, &sa, nullptr) == 0);

    std::atomic_ref<int> atomic(*shared_int1);
    ASSERT(ptrace(PTRACE_TRACEME, 0, 0, 0) == 0);
    atomic.fetch_add(1);
    ASSERT(raise(SIGPWR) == 0);
    atomic.fetch_add(1);
...
int tracer_main(pid_t tracee) {
    ...
    ASSERT(WSTOPSIG(status) == SIGPWR);
    ASSERT((status >> 8) == SIGPWR);
