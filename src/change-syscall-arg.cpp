...
int tracer_main(pid_t tracee) {
    ...
    ASSERT(info.entry.nr == SYS_exit_group);
    ASSERT(info.entry.args[0] == 42);
    // Change exit value to 0
#ifdef __x86_64__
    unsigned long offset = offsetof(user_regs_struct, rdi);
#else
    unsigned long offset = offsetof(user_regs_struct, ebx);
#endif
    ASSERT(syscall(SYS_ptrace, PTRACE_POKEUSER, tracee, offset, 0) == 0);
